...
//		if (1 || Is_Game_Allowed( find_info.cFileName ) ) {
		if ( Is_Game_Allowed( find_info.cFileName ) ) {
			...
			if (item_index >= 0) {
				...
				list_ctrl->Set_Entry_Data (item_index, 0, (LPARAM)new FILETIME(local_time));
				list_ctrl->Set_Entry_Data (item_index, 1, (LPARAM)new StringClass(file_path));
				list_ctrl->Set_Entry_Data (item_index, 2, (LPARAM)new StringClass(find_info.cFileName));
			}
		}
	}
