...
{
	const char* err_string = DXGetErrorStringA(res);

	if (err_string != nullptr) {
		WWDEBUG_SAY((err_string));
	}
...
bool DX8Wrapper::Create_Device(void)
{
	...
	if (FAILED( D3DInterface->GetAdapterIdentifier(CurRenderDevice, D3DENUM_WHQL_LEVEL,&CurrentAdapterIdentifier))) {
		return false;
	}
...
bool DX8Wrapper::Find_Color_Mode(D3DFORMAT colorbuffer, int resx, int resy, UINT *mode)
{
	...
	modemax=D3DInterface->GetAdapterModeCount(D3DADAPTER_DEFAULT, D3DFMT_A8R8G8B8);

	i=0;

	while (i<modemax && !found)
	{
		D3DInterface->EnumAdapterModes(D3DADAPTER_DEFAULT, D3DFMT_A8R8G8B8, i, &dmode);
	...
	unsigned short vertex_count)
{
	...
	// NOTE: have to make sure to call IDirect3DDevice9::SetVertexShader(NULL) if we set a VShader previously
...
const char* DX8Wrapper::Get_DX8_Sampler_State_Name(D3DSAMPLERSTATETYPE state)
{
	switch (state) {
...
void DX8Wrapper::Get_DX8_Sampler_State_Value_Name(StringClass& name, D3DSAMPLERSTATETYPE state, unsigned value)
{
	switch (state) {
