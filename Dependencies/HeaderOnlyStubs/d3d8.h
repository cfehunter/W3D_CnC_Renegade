...
#define D3DSGR_NO_CALIBRATION 1
#endif

#define D3DX_DEFAULT HRESULT(-1)

#define D3DX_FILTER_NONE 0
#define D3DX_FILTER_TRIANGLE 1
...
//#CFE_TODO: Fix these when D3D8 has been fixed. Infact delete this header!
...
using D3DXVECTOR3 = Vector3;
using D3DXVECTOR4 = Vector4;
...
#define MAXD3DDECLLENGTH 255
#endif

inline HRESULT D3DXGetErrorStringA(unsigned res, char* buffer, size_t size)
{
    snprintf(buffer, size, "Fix Me");
    return S_OK;
}

inline unsigned D3DXGetFVFVertexSize(unsigned FVF) { return 0; }

inline unsigned D3DXCreateTexture(IDirect3DDevice8* dev, unsigned int width, unsigned int height, int mips, int usage, int format, int pool, IDirect3DTexture8** textureOut)
{
    return 0;
}


inline unsigned D3DXCreateTextureFromFileExA(IDirect3DDevice8* dev, const char* file, int flags, int more_flags, int mips, int something, int format, int pool, int minFilter, int magFilter, int something_else, void* a, void* b, IDirect3DTexture8** textureOut)
{
    return 0;
}

inline unsigned D3DXLoadSurfaceFromSurface(IDirect3DSurface8* dest, void* something, void* somethingElse, IDirect3DSurface8* src, void* something3, void* something4, int filter, int something5)
{
    return 0;
}

inline unsigned D3DXFilterTexture(IDirect3DTexture8* texture, void* something, int somethingElse, int filter)
{
    return 0;
}

inline D3DXVECTOR4* D3DXVec3Transform(D3DXVECTOR4* result, const D3DXVECTOR3* v, const D3DXMATRIX* m)
{
    return result;
}

inline D3DXMATRIX* D3DXMatrixTranspose(D3DXMATRIX* io, const D3DXMATRIX* src)
{
    return io;
}
